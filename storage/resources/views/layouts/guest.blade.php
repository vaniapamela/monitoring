...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>AgroMonitor | Autentikasi Sistem</title>
            ...
            <div class="w-full sm:max-w-md mt-6 bg-white/90 backdrop-blur-xl shadow-2xl border border-white/20 rounded-[2.5rem] p-8 sm:p-10 transition-all">
                ...
                <div class="text-sm">
                    <form method="POST" action="{{ route('register') }}">
                        ...
                        <div>
                            <x-input-label for="name" value="Name" class="text-xs font-bold text-slate-700 uppercase tracking-wider" />
                            <x-text-input id="name" class="block mt-1 w-full border-slate-200 focus:border-emerald-500 focus:ring-emerald-500 rounded-xl shadow-sm text-sm" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" />
                            <x-input-error :messages="$errors->get('name')" class="mt-2" />
                        </div>

                        <div class="mt-4">
                            <x-input-label for="email" value="Email" class="text-xs font-bold text-slate-700 uppercase tracking-wider" />
                            <x-text-input id="email" class="block mt-1 w-full border-slate-200 focus:border-emerald-500 focus:ring-emerald-500 rounded-xl shadow-sm text-sm" type="email" name="email" :value="old('email')" required autocomplete="username" />
                        ...
                        <div class="mt-4">
                            ...
                            <div class="relative mt-1">
                                ...
                                <button type="button" onclick="togglePassword('password', 'eye-icon-1')" class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-emerald-500 border-none bg-transparent cursor-pointer">
                        ...
                        <div class="mt-4">
                            <x-input-label for="password_confirmation" value="Confirm Password" class="text-xs font-bold text-slate-700 uppercase tracking-wider" />
                            <div class="relative mt-1">
                                <x-text-input id="password_confirmation" class="block w-full pr-10 border-slate-200 focus:border-emerald-500 focus:ring-emerald-500 rounded-xl shadow-sm text-sm"
                                                type="password"
                                                name="password_confirmation" required />
                                
                                <button type="button" onclick="togglePassword('password_confirmation', 'eye-icon-2')" class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-emerald-500 border-none bg-transparent cursor-pointer">
                        ...
                        <div class="flex flex-col gap-4 items-center justify-end mt-8">
                            ...
                            <a class="text-xs font-bold text-slate-400 hover:text-emerald-600 transition-colors no-underline uppercase tracking-wider" href="{{ route('login') }}">
        ...
        <script>
            function togglePassword(inputId, iconId) {
                ...
                if (passwordInput.type === 'password') {
                    ...
                    // Ganti ikon mata terbuka normal
                    eyeIcon.innerHTML = `
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                    `;
